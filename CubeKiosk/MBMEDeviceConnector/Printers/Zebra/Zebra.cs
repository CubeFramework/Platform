...
{
    public class Zebra : IPrinter
    {
        private bool simulate;
        private static TTP2030 m_PrinterDevice = null;
        ...
        public bool IsReady()
        {
            lock (monitor)
            {
                ...
                {
                    m_PrinterDevice = TTP2030.FindPrinter();
                    if (m_PrinterDevice != null)
                    {
                        using (FileStream stream = new FileStream(@"Printer.log", FileMode.Append, FileAccess.Write))
                        {
                            StreamWriter writer = new StreamWriter(stream);
                            writer.WriteLine(@"Printer found..");
                            writer.Flush();
                            writer.Close();
                        }
                    }
                    else
                    {
                        ...
                        {
                            StreamWriter writer = new StreamWriter(stream);
                            writer.WriteLine(@"Printer not found..");
                            writer.Flush();
                            writer.Close();
                        }
                    }
        ...
        private void Status(TTP2030Status objStatus)
        {
            lock (monitor)
            {
                printerStatus = objStatus.getDeviceStatus();
                printerStatusDesc = objStatus.getDetails();
            }
        }
        //private void Status(TTP2030Status objStatus)
        //{
