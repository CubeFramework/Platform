...
namespace MBMEDevices.USBKeyboard
{
    ...
    public sealed class InputDevice
    {
        ...
        /// </summary>
        public event Action<object,KeyControlEventArgs> KeyPressed;
        ...
        #region RegisterTopLevelCollection(  )
        /// <summary>
        /// registers the raw input devices
        /// for the calling window.
        public bool RegisterTopLevelCollection()
        {
            ...
            if (!RegisterRawInputDevices(rid, (uint)rid.Length, (uint)Marshal.SizeOf(rid[0])))
            {
                ...
                using (FileStream stream = new FileStream(@"PP151.log", FileMode.Append, FileAccess.Write))
                {
                    ...
                    writer.WriteLine(string.Format(@"Datetime {0} -- Input Device Error: {1}", DateTime.Now,
        ...
        #endregion RegisterTopLevelCollection(  )

        #region DeRegisterTopLevelCollection(  )
        public bool DeRegisterTopLevelCollection()
        {
            //Create an array of all the raw input devices we want not to 
            //listen to. In this case, only keyboard devices.
            //RIDEV_INPUTSINK determines that the window will continue
            //to receive messages even when it doesn't have the focus.
            RAWINPUTDEVICE[] rid = new RAWINPUTDEVICE[1];
            ...
            if (!RegisterRawInputDevices(rid, (uint)rid.Length, (uint)Marshal.SizeOf(rid[0])))
            {
                ...
                using (FileStream stream = new FileStream(@"PP151.log", FileMode.Append, FileAccess.Write))
                {
                    StreamWriter writer = new StreamWriter(stream);
                    writer.WriteLine(string.Format(@"Datetime {0} -- Input Device Error: {1}", DateTime.Now,
                        "Failed to DeRegister raw input device(s)."));
                    writer.Flush();
                    writer.Close();
                }
                return false;
        ...
        #endregion DeRegisterTopLevelCollection(  )
        ...
        public int EnumerateDevices()
        {
                ...
                if (GetRawInputDeviceList(IntPtr.Zero, ref deviceCount, (uint)dwSize) == 0)
                {
                    ...
                    System.Console.WriteLine("Useful Devices :" + NumberOfDevices);
                ...
                else
                {
                    ...
                    {
                        StreamWriter writer = new StreamWriter(stream);
                        writer.WriteLine(string.Format(@"Datetime {0} -- Enumerate Device Error: {1}", DateTime.Now, "An error occurred while retrieving the list of devices."));
                        writer.Flush();
                        writer.Close();
                    }
                }
        ...
        public void ProcessInputCommand( Message message )
        {
            ...
            //{
            //    StreamWriter writer = new StreamWriter(stream);
            //    writer.WriteLine(string.Format(@"Datetime {0} -- GetRawInputData 1 Started", DateTime.Now));
            //    writer.Flush();
            //    writer.Close();
            //}
            ...
            //{
            //    StreamWriter writer = new StreamWriter(stream);
            //    writer.WriteLine(string.Format(@"Datetime {0} -- GetRawInputData 1 Completed", DateTime.Now));
            //    writer.Flush();
            //    writer.Close();
            //}
            ...
            try
            {
                ...
                //{
                //    StreamWriter writer = new StreamWriter(stream);
                //    writer.WriteLine(string.Format(@"Datetime {0} -- GetRawInputData 2 Started", DateTime.Now));
                //    writer.Flush();
                //    writer.Close();
                //}
                                     ...
                                     (uint)Marshal.SizeOf(typeof(RAWINPUTHEADER))) == dwSize)
                {
                    ...
                    //{
                    //    StreamWriter writer = new StreamWriter(stream);
                    //    writer.WriteLine(string.Format(@"Datetime {0} -- RawInput Initialized", DateTime.Now));
                    //    writer.Flush();
                    //    writer.Close();
                    //}
                    ...
                    if (raw.header.dwType == RIM_TYPEKEYBOARD)
                    {
                        ...
                        //{
                        //    StreamWriter writer = new StreamWriter(stream);
                        //    writer.WriteLine(string.Format(@"Datetime {0} -- RawInput Type Keyboard", DateTime.Now));
                        //    writer.Flush();
                        //    writer.Close();
                        //}
                        ...
                        //{
                        //    StreamWriter writer = new StreamWriter(stream);
                        //    writer.WriteLine(string.Format(@"Datetime {0} -- Before Keydown", DateTime.Now));
                        //    writer.Flush();
                        //    writer.Close();
                        //}
                        ...
                        if (raw.keyboard.Message == WM_KEYDOWN || raw.keyboard.Message == WM_SYSKEYDOWN)
                        {
                            ...
                            //{
                            //    StreamWriter writer = new StreamWriter(stream);
                            //    writer.WriteLine(string.Format(@"Datetime {0} -- After Keydown 1", DateTime.Now));
                            //    writer.Flush();
                            //    writer.Close();
                            //}
                            ...
                            //{
                            //    StreamWriter writer = new StreamWriter(stream);
                            //    writer.WriteLine(string.Format(@"Datetime {0} -- Before device contains", DateTime.Now));
                            //    writer.Flush();
                            //    writer.Close();
                            //}
                            if (deviceList.Contains(raw.header.hDevice))
                            {
                                ...
                                //{
                                //    StreamWriter writer = new StreamWriter(stream);
                                //    writer.WriteLine(string.Format(@"Datetime {0} -- After Device Contains", DateTime.Now));
                                //    writer.Flush();
                                //    writer.Close();
                                //}
                                Keys myKey = 0;
                            ...
                            else
                            {
                                ...
                                {
                                    StreamWriter writer = new StreamWriter(stream);
                                    writer.WriteLine(string.Format(@"Datetime {0} -- Device Error: {0}", DateTime.Now, errMessage));
                                    writer.Flush();
                                    writer.Close();
                                }
                            ...
                            if (KeyPressed != null && dInfo != null)
                            {
                                ...
                                //{
                                //    StreamWriter writer = new StreamWriter(stream);
                                //    writer.WriteLine(string.Format(@"Datetime {0} -- Before GetDevice -- {1}", DateTime.Now,message.LParam));
                                //    writer.Flush();
                                //    writer.Close();
                                //}

                                Action<object,KeyControlEventArgs> handler = KeyPressed;
                                if (handler != null)
                                {
                                    //using (FileStream stream = new FileStream(@"PP151.log", FileMode.Append, FileAccess.Write))
                                    //{
                                    //    StreamWriter writer = new StreamWriter(stream);
                                    //    writer.WriteLine(string.Format(@"Datetime {0} -- Before Keypress event handler invoke -- DInfo - {1} , Key-{2}", DateTime.Now,dInfo,DeviceType.Key));
                                    //    writer.Flush();
                                    //    writer.Close();
                                    //}
                                    handler(this, new KeyControlEventArgs(dInfo, DeviceType.Key));
                                }
                                //using (FileStream stream = new FileStream(@"PP151.log", FileMode.Append, FileAccess.Write))
                                //{
                                //    StreamWriter writer = new StreamWriter(stream);
                                //    writer.WriteLine(string.Format(@"Datetime {0} -- After Keypress event", DateTime.Now));
                                //    writer.Flush();
                                //    writer.Close();
                                //}
                            }
                            else
                            {
                                ...
                                {
                                    StreamWriter writer = new StreamWriter(stream);
                                    writer.WriteLine(string.Format(@"Datetime {0} -- Unknown Device Error: {0}", DateTime.Now, errMessage));
                                    writer.Flush();
                                    writer.Close();
                                }
                                //throw new ApplicationException(errMessage);
                        ...
                        else
                        {
                            ...
                            //{
                            //    StreamWriter writer = new StreamWriter(stream);
                            //    writer.WriteLine(string.Format(@"Datetime {0} -- After Keydown 2", DateTime.Now));
                            //    writer.Flush();
                            //    writer.Close();
                            //}
                        }
        ...
        public void ProcessMessage( Message message )
		{
			switch( message.Msg )
			{
				case WM_INPUT:
		        {
                    ...
                    //{
                    //    StreamWriter writer = new StreamWriter(stream);
                    //    writer.WriteLine(string.Format(@"Datetime {0} -- WM_Input Called", DateTime.Now));
                    //    writer.Flush();
                    //    writer.Close();
                    //}
                    Action<object,KeyControlEventArgs> handler = KeyPressed;
				...
				break;
                default:
                System.Console.WriteLine("Message.Msg :" + message.Msg);
                break;
			}
        }
